#!/bin/bash
#SBATCH --job-name=track_a_gpt2m_full
...
#SBATCH --output=/home/sukrit.koirala/ondemand/upload_me/RegionTokenizer/scripts/RL/logs/track_a_gpt2m_full-%j.out
#SBATCH --error=/home/sukrit.koirala/ondemand/upload_me/RegionTokenizer/scripts/RL/logs/track_a_gpt2m_full-%j.err
#
# Track A: GPT-2 Medium Second-Model Replication — Full Run
...
echo "========================================================"
echo " Track A: GPT-2 Medium Second-Model Full Run"
echo " output:  ${OUTPUT}"
echo " $(date)"
echo "========================================================"
echo ""
...
print('  Checking gpt2-medium ...')
cfg = GPT2LMHeadModel.from_pretrained('gpt2-medium').config
...
echo ""
echo "========================================================"
echo " GPT-2 medium full run complete.  $(date)"
echo "========================================================"
echo ""
...
if os.path.isfile(sum_path):
    ...
    for r in s.get("results_by_seed", []):
        print(f"  Seed {r.get('seed')}: {r.get('overall')}  "
              f"best_q={r.get('best_q_nll')}  "
              f"gpt_nll={r.get('gpt_nll')}")
        fr = r.get("full_raw")
        if fr:
            print(f"    full_raw:  Q={fr.get('full_raw_q_nll')}  "
                  f"fixed={fr.get('full_raw_fixed_nll')}")
...
echo "  Summary:    ${OUTPUT}/model_replication_summary.json"
