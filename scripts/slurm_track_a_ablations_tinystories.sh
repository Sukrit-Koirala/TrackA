#!/bin/bash
#SBATCH --job-name=track_a_abl_ts
...
#SBATCH --output=/home/sukrit.koirala/ondemand/upload_me/RegionTokenizer/scripts/RL/logs/track_a_abl_ts-%j.out
#SBATCH --error=/home/sukrit.koirala/ondemand/upload_me/RegionTokenizer/scripts/RL/logs/track_a_abl_ts-%j.err
#
# Track A: State-Object Ablations — TinyStories / GPT-2 small
# All 11 variants, seeds 42/123/999, budgets 5k/10k/25k/50k, 200k Q samples
# ─────────────────────────────────────────────────────────────────────────────
...
# ── Config ────────────────────────────────────────────────────────────────────
OUTPUT_BASE="outputs_track_a_offline_paper"
...
# ── Data source ───────────────────────────────────────────────────────────────
# Same source used for all seeds (mirrors slurm_track_a_offline_paper.sh SOURCE).
# controller_train.pt / val.pt / datastore.pt live here.
DATA_SOURCE="scale_200k_seed42"

echo "========================================================"
echo " Track A: State-Object Ablations (TinyStories / GPT-2 small)"
...
[ ! -f "${DATA_SOURCE}/states/controller_train.pt" ] && { echo "ERROR: controller_train.pt not found in ${DATA_SOURCE}/states/"; exit 1; }
echo "  [OK] ${DATA_SOURCE}/states/controller_train.pt"
...
base = Path("outputs_track_a_offline_paper")
seeds = [42, 123, 999]
rows = []
...
out_csv = rep / "ablations_tinystories_all_seeds.csv"
df.to_csv(out_csv, index=False)
print(f"\n  Saved: {out_csv}  ({len(df)} rows total)")

# Cross-seed mean Q NLL per variant
...
if len(orig) > 0:
    ...
    with open(rep / "ablations_tinystories_cross_seed_verdict.json", "w") as f:
        json.dump({
            "dataset": "TinyStories",
            "model": "gpt2",
            "seeds": seeds,
    ...
    print(f"  Saved: {rep}/ablations_tinystories_cross_seed_verdict.json")
PYEOF

echo ""
echo "[done] Outputs: ${OUTPUT_BASE}/"
echo "  Per-seed: ${OUTPUT_BASE}/seed{42,123,999}/ablations/"
echo "  Reports:  ${OUTPUT_BASE}/reports/ablations_tinystories_*.{csv,json}"
